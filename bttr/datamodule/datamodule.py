...
def data_iterator(
    ...
    data.sort(key=lambda x: x[1].size[0] * x[1].size[1])    # 根据面积从小到大进行排序

    i = 0
    for fname, fea, lab in data:
        size = fea.size[0] * fea.size[1]    # 计算面积
        fea = transforms.ToTensor()(fea)    # 图片转tensor
        if size > biggest_image_size:   # 这里主要是为了进行比较计算最大面积
