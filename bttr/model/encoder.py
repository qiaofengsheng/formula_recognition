...
class DenseNet(nn.Module):
    ...
    def forward(self, x, x_mask):   # 这部分的是进行densenet特征提取，对其图片padding后的mask图也进行下采样
        out = self.conv1(x)
        out = self.norm1(out)
...
class Encoder(pl.LightningModule):
    ...
    def forward(
        ...
        """
        # extract feature
        feature, mask = self.model(img, img_mask)   # densenet特征提取，同时对padding的mask也进行下采样
        feature = self.feature_proj(feature)

        # proj
        feature = rearrange(feature, "b d h w -> b h w d")  #  nchw -> nhwc
        feature = self.norm(feature)    # layerNormal

        # positional encoding
        feature = self.pos_enc_2d(feature, mask)    # 添加卷积特征图的位置编码

        # flat to 1-D
        feature = rearrange(feature, "b h w d -> b (h w) d")    # 将宽高进行合并 nhwc->hsc
        mask = rearrange(mask, "b h w -> b (h w)")  # mask同样将宽高进行合并 nhw->ns
        return feature, mask
